use crate::{
    app_db::Epi2MeAnalysis, depme_nextflow::NextflowAssetWorkflow,
    nextflow::nextflow_log_item::NxfLogItem, workflow::Workflow,
};
use polars::prelude::*;
...
pub fn workflow_vec_to_df(wfs: Vec<Workflow>) -> DataFrame {
    struct_to_dataframe!(wfs, [project, name, version]).unwrap()
}
...
pub fn nf_wf_vec_to_df(vec: Vec<NextflowAssetWorkflow>) -> DataFrame {
    struct_to_dataframe!(vec, [workflow, path, version]).unwrap()
}
...
pub fn filter_df_by_value(
    ...
    return df
        .clone()
        .lazy()
        .filter(col(column).is_in(lit(Series::from_iter(vec![String::from(value)]))))
        .collect();
}
