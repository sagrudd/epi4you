...
pub fn get_cli_setup() -> Command {
    ...
    let my_command = Command::new(NEXTFLOW_RUN)
        ...
        .arg(arg!(--nxf_bin "path to nextflow binary (if not obvious)").action(ArgAction::Set).required(false).value_parser(value_parser!(String)))
        .arg(arg!(--nxf_work "path to nextflow work folder").action(ArgAction::Set).required(false).value_parser(value_parser!(String)))
        .arg(arg!(--runid "export EPI2ME analysis by run_name").action(ArgAction::Set).required(false).value_parser(value_parser!(String)))
...
pub fn process_clicapture_command(args: &ArgMatches, tempdir: &TempDir) -> Result<(), Epi4youError> {
    ...
    let list = args.get_one::<bool>("list").unwrap().to_owned();
    ...
    if nxf_workdir.is_none() {
        ...
        log::info!("Setting nextflow workdir to cwd [{:?}]", nxf_workdir.clone().unwrap());
    }

    let nextflow_run_folder = NextFlowResultFolder::init(PathBuf::from(nxf_workdir.clone().unwrap()), nxf_bin)?;
    ...
    if list {
        ...
        if runid.is_none() {
            log::error!("EPI2ME analysis twome archiving requires a --runid identifier (run_name)");
            return Err(Epi4youError::AdditionalParameterRequired);
        } 

        if twome.is_none() {
            log::error!("EPI2ME analysis twome archiving requires a --twome identifier (archive to write)");
            return Err(Epi4youError::AdditionalParameterRequired);
        } 
